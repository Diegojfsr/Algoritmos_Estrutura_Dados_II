...
#Considere variância homogenea para resolver o exercicio 
...
#NÃO SERÁ POSSÍVEL RESOLVER A LISTA 2 USANDO t.text  para os exercicios 1 e 2 
#POIS NAO FOI INFORMADO OS VALORES DAS AMOSTRAS 
...
################################################################################
#No exercicio 3 foram dadas os valores das amostras logo sera possivel revolver 
...
rownames(df) <- c("rise de Ansiedade", "Depressao", "Se sente Infeliz")
...
#In the image above, it’s evident that there are an association between the column Durante_Pandemia and the row Dorme Mal.
#There is a strong positive association between the column Durante_Pandemia and the row Se assusta Facilmente


#2)Negative residuals are in red. This implies a repulsion (negative association) 
#between the corresponding row and column variables. 
#For example the column Durante_Pandemia are negatively associated (~ “not associated”) with the row Dorme Mal. 
#There is a repulsion between the column Pre Pandemia and, the row Se assusta Facilmente
